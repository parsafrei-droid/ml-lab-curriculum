...
`experiments/second_wave/train_short.sh`:
...
(`experiments/second_wave/submit.sh`: `gpu_h100`, `--time=12:00:00`, 2 configs × 3 seeds) and
...
print(sum(float(list(csv.DictReader(open(f)))[-1]['cum_time_s'])
          for f in glob.glob('experiments/second_wave/results/kaggle_run/*log.csv'))/3600, 'h')"
```
