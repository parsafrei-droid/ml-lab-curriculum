...
NOTE = [
    ...
    "binary. 10 random learning rates from the",
    "paper's own search space, each trained twice",
    "(baseline vs early-ramp curriculum), 3 seeds.",
    "Band = seed min-max.",
    "",
    "The y-axis is the cheap per-checkpoint probe,",
    "not TabArena. For a binary model it reduces to",
    "one easy dataset (breast_cancer), which is why",
    "the values sit near 0.99. It tracks convergence",
    "speed, not benchmark quality.",
    "",
    "At a well-tuned lr the curriculum is steady",
    "from step 200 while the baseline is erratic and",
    "catches up only late. On real TabArena the final",
    "gap at this lr is only +0.003, so here the",
    "curriculum buys speed, not a higher ceiling.",
]
...
def make(fname):
    ...
    ax.set_ylabel("ROC-AUC on the small OpenML probe (breast_cancer)")
