...
# 30-min cap and runs in parallel. Each batch-1 5k run is ~6 min.
#
#   sbatch scripts/lr_sweep_one.sh <scenario> <lr> <seed> <epochs>
#
#SBATCH --job-name=lr1
...
set -e
SCENARIO="$1"; LR="$2"; SEED="$3"; EPOCHS="$4"
...
python scripts/run.py --config experiments/configs/$SCENARIO.yaml \
  --lr $LR --name $name --seed $SEED --epochs $EPOCHS
