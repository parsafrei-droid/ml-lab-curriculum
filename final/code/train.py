...
# nanoTabPFN attends twice per layer, once across rows and once across columns, and the
# row attention runs per column. So a table with few features is genuinely cheaper, which
# is the whole reason the feature curriculum front-loads cheap compute. Times 3 for
...
def main():
    ...
    # We load the dump and pick an order. Everything after this point is identical
    # across runs, so the order is the only variable in the experiment.
    ...
    # One pass over the dump. The pool is sized so that steps x accum uses every table
    # exactly once, so a run is a single sweep with no repeats.
