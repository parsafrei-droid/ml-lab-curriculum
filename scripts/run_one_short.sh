...
#
#   sbatch scripts/run_one_short.sh <config> <name> <seed> [steps]
...
set -e
CONFIG="$1"; NAME="$2"; SEED="$3"; STEPS="$4"
...
echo "############################## $NAME ##############################"
if [ -n "$STEPS" ]; then
  python scripts/run.py --config "$CONFIG" --name "$NAME" --seed "$SEED" --steps "$STEPS"
else
